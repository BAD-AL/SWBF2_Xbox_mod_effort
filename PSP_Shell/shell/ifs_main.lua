...
ifsmain_vbutton_layout = {
	...
	buttonlist = {
		...
		{tag = "psptut", string = "ifs.main.psptut"}
	},
	title = "ifs.main.title"
}

function SetHelpTextVis(arg1)
	print("SetHelpTextVis ", arg1)
	local this = ifs_main
	IFObj_fnSetVis(this.Helptext_Back, arg1)
	IFObj_fnSetVis(this.Helptext_Accept, arg1)
end
	...
	if ((gDemoBuild) and (not gDemoHasMP)) then
		...
		if ((this.LastXLiveSilentUStr ~= ShowUStr) or (not this.iLastXLiveSilentTimer)) then
			...
			elseif (this.iLastXLiveSilentTimer == 0) then
				local fFadeTime = 2.5
				AnimationMgr_AddAnimation(this.LoginInfoWindow.ShowText, {fTotalTime = fFadeTime, fStartAlpha = 1, fEndAlpha = 0})
...
function ifs_mp_main_fnCheckDone()
	return ScriptCB_IsAdhocConnected()
end

function ifs_mp_main_fnOnSuccess()
	print("ifs_mp_main_fnOnSuccess")
	local this = ifs_main
	...
	ifs_main_GotoLoginScreen(this, ifs_mp_main)
end

function ifs_mp_main_fnOnFail_done()
	if (gPlatformStr == "PSP") then
		SetHelpTextVis(1)
	end
end

function ifs_mp_main_fnOnFail()
	print("ifs_mp_main_fnOnFail")
	local this = ifs_main
	Popup_Busy:fnActivate(nil)
	Popup_Ok_Large.fnDone = ifs_mp_main_fnOnFail_done
	IFText_fnSetString(Popup_Ok_Large.title, "ifs.mp.connecterror")
	Popup_Ok_Large:fnActivate(1)
end

function ifs_mp_main_fnOnCancel()
	print("ifs_mp_main_fnOnCancel")
end
--function  ifs_mp_main_fnCheckDone
...
ifs_main =
	...
	Enter = function(this, bFwd)
		...
		elseif (bFwd and ScriptCB_InMultiplayer()) then
			if (gPlatformStr ~= "PSP") then
				...
				if (gPlatformStr == "PC") then
					if (ScriptCB_GetAmHost()) then --if (ScriptCB_GetWasHost()) then
		...
		this.buttons.split.hidden =
			(gPlatformStr == "PC") or (gDemoBuild and (not gDemoHasSplitscreen) or gPlatformStr == "PSP")
	...
	--[[fnJumpToMPMain = function(this)
		...
		if (this.CurButton == "psptut") then
			ifs_movietrans_PushScreen(ifs_psptutorial)
		elseif (this.CurButton == "sp") then
			--
		...
		elseif (this.CurButton == "mp") then
			...
			if (not bPresent) then
				...
				if (gPlatformStr == "PSP") then
					IFText_fnSetString(Popup_Ok_Large.title, "ifs.error.netcable_removed")
				else
				...
				elseif (gPlatformStr == "PSP") then
					...
					ScriptCB_SetConnectType("lan")
					ScriptCB_ConnectToLobby()
					if (ScriptCB_IsPopupOpen()) then
						ScriptCB_ClosePopup()
					end
					SetHelpTextVis(nil)
					Popup_Busy.fnCheckDone = ifs_mp_main_fnCheckDone
					Popup_Busy.fnOnSuccess = ifs_mp_main_fnOnSuccess
					Popup_Busy.fnOnFail = ifs_mp_main_fnOnFail
					Popup_Busy.fnOnCancel = ifs_mp_main_fnOnCancel

					Popup_Busy.bNoCancel = 1
					Popup_Busy.fTimeout = 8
					IFText_fnSetString(Popup_Busy.title, "ifs.mp.connecting")
...
if (gPlatformStr == "PSP") then
	ifs_main.logo_temp.localpos_l = -192
	ifs_main.logo_temp.localpos_t = 20
	ifs_main.logo_temp.localpos_r = 192
	ifs_main.logo_temp.localpos_b = 165
end
